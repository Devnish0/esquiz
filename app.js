...
(function () {
  ...
  function initFontSwitcher() {
    ...
    const saved = localStorage.getItem('quiz-font') || 'zeroarea';
    select.value = saved;
    applyFont(saved);
  ...
  function applyFont(key) {
    document.body.style.fontFamily = fontMap[key] || fontMap.zeroarea;
    document.documentElement.style.setProperty('--current-font', fontMap[key] || fontMap.zeroarea);
  }
  ...
  function initParticles() {
    ...
    for (let i = 0; i < 50; i++) {
      const greenShade = Math.random() > 0.5 ? '74, 222, 128' : '134, 239, 172';
      particles.push({
